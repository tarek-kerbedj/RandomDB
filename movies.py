...
year = st.sidebar.slider(
    "movie release year", min_value=1900, max_value=2019, value=(1950, 2019)
)
...
if random_pick:
    ...
    try:
        ...
        res = imdb.get_by_id(
            filtered_movies.iloc[pick]["imdb_title_id"].replace("-i", "tt")
        )
        im = json.loads(res)["poster"]
        col6.header(
            filtered_movies.iloc[pick]["original_title"]
            + " "
            + "("
            + str(filtered_movies.iloc[pick]["year"])
            + ")"
        )
        ...
        col6.image(
            ...
            caption="no poster found",
        )

    except IndexError:
        col5, col6, col7 = st.columns([1, 2, 1])
        col6.image(
            "megamind.png", caption="Try (unironically) lowering your expectations."
        )
