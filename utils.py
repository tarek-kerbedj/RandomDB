...
def filter_movies(movies, genre, language, rating, year):
    expr = movies[(movies['year'] > year[0]) & (movies['year'] < year[1]) & (movies['avg_vote'] > rating)]
    if language == 'unspecified' and genre == 'unspecified':
        ...
        ls1 = movies[(movies['year'] > year[0]) & (movies['year'] < year[1]) & (movies['avg_vote'] > rating) & (movies['language'] == language) & (movies['genre'].str.match(genre) == True)]
    return ls1